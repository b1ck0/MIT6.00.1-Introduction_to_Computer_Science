...
def getWordScore(word, n):
    ...
    # getting the scores due to the scrambled letter points
    for char in word:
        score += SCRABBLE_LETTER_VALUES[char]
        
    #getting additional points for word length
    score *= len(word)
    
    #getting the bonus points if all n cards have been used for building a word
    if len(word) == n :
        score += 50
        
    assert isinstance(score,int), "score is not a integer"
    return score
#
...
def isValidWord(word, hand, wordList):
    ...
    # checking if the word is in the word list
